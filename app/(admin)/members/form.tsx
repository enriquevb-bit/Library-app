...
export default function MemberFormScreen() {
  ...
  useEffect(() => {
    if (isEditing) loadExistingMember();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    if (!isEmail(email.trim())) {
      alert({ title: 'Error', message: 'Introduce un email válido (ej. nombre@dominio.com).' });
      return;
    }
    ...
    setLoading(true);
    try {
      if (isEditing) await updateMember(id!, member);
      else await createMember(member);
      router.back();
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.form}>
        ...
        <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Nombre completo" placeholderTextColor={colors.textMuted} />
        ...
        <View style={styles.stateRow}>
          {STATES.map((state) => (
            <TouchableOpacity
              key={state}
              style={[styles.stateChip, memberState === state && styles.stateChipActive]}
              onPress={() => setMemberState(state)}
            >
              <Text style={[styles.stateChipText, memberState === state && styles.stateChipTextActive]}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitText}>{isEditing ? 'Guardar cambios' : 'Crear miembro'}</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  stateRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 4 },
  stateChip: {
